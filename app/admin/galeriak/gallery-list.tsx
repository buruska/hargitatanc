...
export function GalleryList({ galleries }: { galleries: Gallery[] }) {
  ...
  return <>
    ...
    <div className="mt-6 grid gap-4">
      {filtered.map((gallery) => <article className={`${panel} grid gap-4 p-4 min-[720px]:grid-cols-[112px_1fr] min-[720px]:items-center`} key={gallery.id}>
        <Image alt="" className="aspect-[4/3] w-full border-2 border-charcoal object-cover" height={84} src={gallery.coverImageUrl} width={112} />
        <div className="flex flex-col gap-3 min-[760px]:flex-row min-[760px]:items-center min-[760px]:justify-between">
          <div><h2 className="font-serif text-2xl font-bold leading-tight">{gallery.title}</h2><p className="mt-1 text-sm font-extrabold text-muted">{gallery.galleryImages.length} kép · {gallery.galleryIsPublished ? "Publikálva" : "Elrejtve"}</p></div>
