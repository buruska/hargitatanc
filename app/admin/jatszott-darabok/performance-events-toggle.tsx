...
const galleryFormInitialState: CreateGalleryState = {};

const albumDateFormatter = new Intl.DateTimeFormat("hu-RO", {
  dateStyle: "long",
  timeZone: "Europe/Bucharest",
});
  ...
  if (showUploadAction) {
    return (
      <>
        ...
        <UploadPerformanceImagesModal event={event} isOpen={isImagesOpen} onClose={() => setIsImagesOpen(false)} />
      </>
    );
  }
...
function UploadPerformanceImagesModal({
  event,
  isOpen,
  onClose,
}: Readonly<{
  ...
  const [state, formAction, isPending] = useActionState(createPerformanceGalleryAction, galleryFormInitialState);
  ...
  const titleId = useId();
  const defaultAlbumTitle = `${event.performanceTitle} – ${event.location} – ${albumDateFormatter.format(new Date(event.startsAt))}`;
  ...
  }, [defaultAlbumTitle, isOpen, onClose]);
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-charcoal/60 px-4 py-8" role="presentation">
      <section aria-labelledby={titleId} aria-modal="true" className={`${panel} w-full max-w-[720px] p-6`} role="dialog">
        ...
        <form action={formAction} className="grid gap-5" ref={formRef}>
          ...
          <div className={step === 1 ? "grid gap-5" : "hidden"}>
            <label className={label}>
              Album neve
              <input className={input} defaultValue={defaultAlbumTitle} name="title" ref={titleRef} required />
            </label>
            <label className={label}>
              Képek
              <input
                accept="image/*"
                className="min-h-[46px] border-2 border-line-strong bg-surface-strong px-3 py-2.5 file:mr-4 file:border-0 file:bg-thread-red file:px-3 file:py-2 file:font-extrabold file:text-white"
                multiple
                name="galleryImages"
                ref={imagesRef}
                required
          ...
          {step === 2 ? (
            <div className="grid gap-5">
              <div>
                ...
                <p className="mt-1 text-sm font-bold text-muted">Kattints arra a képre, amelyiket az album borítójaként szeretnéd használni.</p>
              </div>
              ...
              <div className="grid max-h-[52vh] grid-cols-2 gap-3 overflow-y-auto pr-1 min-[560px]:grid-cols-3">
                {previews.map((preview, index) => (
                  <button
                    className={`group relative border-4 transition ${coverImageIndex === index ? "border-thread-red" : "border-transparent hover:border-line-strong"}`}
                    key={preview}
                    type="button"
              ...
              <div className="flex flex-col gap-3 min-[520px]:flex-row min-[520px]:justify-end">
                ...
                <button className={buttonPrimary} disabled={isPending} type="submit">
                  {isPending ? "Feltöltés..." : "Album véglegesítése"}
                </button>
              </div>
            </div>
          ) : null}
