...
import { deleteRunningPerformanceGalleryImageAction, type DeleteGalleryImageState } from "./actions";
...
const initialState: DeleteGalleryImageState = {};

export function DeleteGalleryImageModal({ id, imageUrl }: DeleteGalleryImageModalProps) {
  const [state, formAction, isPending] = useActionState(deleteRunningPerformanceGalleryImageAction, initialState);
  ...
  return (
    <>
      <button
        className="inline-flex min-h-8 items-center justify-center border border-[#7f1712] bg-thread-red px-3 py-1 text-xs font-extrabold text-surface-strong transition hover:bg-[#8f1f1a]"
        type="button"
      ...
      {isOpen ? (
        <div className="fixed inset-0 z-50 grid place-items-center bg-charcoal/60 px-4 py-8" role="presentation">
          ...
          >
            ...
            <p className="text-muted">Biztos, hogy törölni akarjuk ezt a képet?</p>
